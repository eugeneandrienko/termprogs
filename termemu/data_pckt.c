...
#include "net.h"
 ...
 /* Функция для расчета CRC по стандартному алгоритму CRC16 */
unsigned short crc16(unsigned char * pcBlock, unsigned short len) {
	static const unsigned short int Crc16Table[256] = {
...
/* Функция шифрует блок данных data, размером size, ключом serv_pubkey.
 ...
 * запросом данных.
 * Возвращает ноль если все в порядке и число != 0 если произошла ошибка.
 */
int create_data_pckt(void * rcvr_addr, struct sockaddr_in * host_addr, RSA * serv_pubkey) {
	/* Поскольку датчиков у нас пока нет - отправим время
	 */
	time_t curr_time = time(NULL);
	char * data = ctime(&curr_time);
	unsigned short int crc_16 = 0;
	void * encdata = NULL;
	unsigned long int encdata_size = 0;
	void * data_pckt = NULL;
	unsigned char * pdata = NULL;
	void * sender = NULL;

	/* вычисляем crc незашифрованного блока данных */
	crc_16 = crc16((unsigned char *)data, strlen(data) + 1);

	/* криптуем блок данных, получаем размер зашифрованного блока */
	if ((encdata = crypt_data(data, strlen(data) + 1, &encdata_size, serv_pubkey)) == NULL) {
		syslog(LOG_ERR, "Error, cannot crypt data block!\n");
		return -1;
	}

	/* Выделяем память под пакет данных */
	if ((data_pckt = malloc(encdata_size + 20)) == NULL) {
		syslog(LOG_ERR, "Error allocating memory for data packet\n");
		return -1;
	}

	/* Заполняем бит типа пакета и адрес получателя */
	pdata = data_pckt;
	*pdata = 0xda;
	pdata++;
	memcpy(pdata, rcvr_addr, 6);
	pdata += 6;

	/* Получаем и заполняем адрес отправителя */
	...
	memcpy(pdata, sender, 6);
	free(sender);
	pdata += 6;

	encdata_size = htonl(encdata_size);
	/* Заполняем поле размера блока данных */
	memcpy(pdata, &encdata_size, sizeof(encdata_size));
	encdata_size = ntohl(encdata_size);
	pdata += sizeof(encdata_size);

	/* Заполняем поле для блока данных */
	memcpy(pdata, encdata, encdata_size);
	pdata += encdata_size;
	free(encdata);

	/* Заполняем поле CRC16 */
	memcpy(pdata, &crc_16, sizeof(crc_16));
	pdata += sizeof(crc_16);

	/* Добавляем null-terminate байт в конец */
	*pdata = 0x00;

	if (send_data(data_pckt, encdata_size + 20) == 13) {
		syslog(LOG_ERR, "Error adding data packet to buffer: No enough memory\n");
		return -1;
	}
