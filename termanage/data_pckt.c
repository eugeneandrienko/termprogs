 ...
 */
int parse_data_pckt(unsigned char * incoming_pckt, unsigned long int incoming_pckt_size,
		void * data_pckt_data, unsigned long int data_pckt_data_size, RSA * serv_privkey) {
	unsigned long int decrypt_blksize = 0;
	void * decrypt_blk = NULL;
	unsigned short int crc = 0;
	char * incoming_string = NULL;

	if ((decrypt_blk = decrypt_data(data_pckt_data, data_pckt_data_size - 3, &decrypt_blksize, serv_privkey)) == NULL) {
		syslog(LOG_ERR, "Error decrypt data block\n");
		return -1;
	}

	memcpy(&crc, data_pckt_data + data_pckt_data_size - 3, 2);
	if (crc != crc16((unsigned char *)decrypt_blk, decrypt_blksize)) {
		syslog(LOG_WARNING, "Crc failed\n");
		return -1;
	}

	if ((incoming_string = malloc(decrypt_blksize + 1)) == NULL) {
		syslog(LOG_ERR, "Cannot allocate memory for string\n");
		return -1;
	}
	memcpy(incoming_string, decrypt_blk, decrypt_blksize);
	incoming_string[decrypt_blksize] = '\0';
